...
void Referee_Class::init(UART_HandleTypeDef * huart,bool use_dma){
	huart_ = huart;
	use_dma_ = use_dma;
}
void Referee_Class::request(){
  if (use_dma_) {
    // dismiss return
    HAL_UARTEx_ReceiveToIdle_DMA(huart_, referee_buff_, REFEREE_BUFF_SIZE);
    ...
    // dismiss return
    HAL_UARTEx_ReceiveToIdle_IT(huart_, referee_buff_, REFEREE_BUFF_SIZE);
  }
}

void Referee_Class::update(uint16_t size) { update(referee_buff_, size); }

void Referee_Class::update(uint8_t * frame_start, uint16_t size){
...
void Referee_Class::send(const uint8_t * data, size_t size){
  if (use_dma_) {
    HAL_UART_Transmit_DMA(huart_, data, size);
  }
  else {
    HAL_UART_Transmit(huart_, data, size, 0xFF);
  }
}