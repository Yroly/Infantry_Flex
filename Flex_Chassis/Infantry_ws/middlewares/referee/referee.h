...
struct __attribute__((packed)) GameRobotHP{
	uint16_t ally_1_robot_HP;    //!< @brief 己方1号英雄机器人血量。若该机器人未上场或者被罚下，则血量为 0
	uint16_t ally_2_robot_HP;    //!< @brief 己方2号工程机器人血量
	uint16_t ally_3_robot_HP;    //!< @brief 己方3号步兵机器人血量
	uint16_t ally_4_robot_HP;    //!< @brief 己方4号步兵机器人血量
	uint16_t reserved;    			//!< @brief 保留位
	uint16_t ally_7_robot_HP;    //!< @brief 己方7号哨兵机器人血量
	uint16_t ally_outpost_HP;    //!< @brief 己方前哨站血量
	uint16_t ally_base_HP;       //!< @brief 己方基地血量
};
...
struct __attribute__((packed)) PowerHeatData{
	uint16_t reserved;              //!< @brief 电源管理模块的 chassis 口输出电压（单位：mV）
	uint16_t reserved_1;              //!< @brief 电源管理模块的 chassis 口输出电流（单位：mA）
	float reserved_2;                 //!< @brief 底盘功率（单位：W）
...
struct __attribute__((packed)) ProjectileAllowance{
	...
	uint16_t projectile_allowance_fortress;//!< @brief 堡垒增益点提供的储备 17mm 弹丸允许发弹量
};
...
struct __attribute__((packed)) RfidStatus{
  ...
  uint32_t friendly_tunnel_road_down : 1;  // bit 26：己方地形跨越增益点（隧道）（靠近己方一侧公路区下方）
  uint32_t friendly_tunnel_road_up : 1;  // bit 27：己方地形跨越增益点（隧道）（靠近己方一侧公路区上方）
  uint32_t friendly_tunnel_trapezoid_low : 1;  //bit 28：己方地形跨越增益点（隧道）（靠近己方梯形高地较低处）
  uint32_t friendly_tunnel_trapezoid_high : 1;  //bit 29：己方地形跨越增益点（隧道）（靠近己方梯形高地较高处）
  uint32_t enemy_tunnel_road_down : 1;  // bit 30：对方地形跨越增益点（隧道）（靠近己方一侧公路区下方）
  uint32_t enemy_tunnel_road_up : 1;  // bit 31：对方地形跨越增益点（隧道）（靠近己方一侧公路区上方）
  uint8_t enemy_tunnel_trapezoid_low : 1;  //bit 0：对方地形跨越增益点（隧道）（靠近己方梯形高地较低处）
  uint8_t enemy_tunnel_trapezoid_high : 1;  //bit 1：对方地形跨越增益点（隧道）（靠近己方梯形高地较高处）
};
...
struct __attribute__((packed)) GroundRobotPosition{
	...
	float reserved;              //!< @brief 己方 5 号步兵机器人位置 x 轴坐标，单位：m
	float reserved_1;            //!< @brief 己方 5 号步兵机器人位置 y 轴坐标，单位：m
};
...
struct __attribute__((packed)) SentryInfo{
  ...
  uint16_t sentry_current_pose : 2;  // bit 12-13      : 哨兵当前姿态，1 为进攻姿态，2 为防御姿态，3 为移动姿态
...
struct __attribute__((packed)) RadarInfo{
  ...
  uint8_t friendly_encryption_level : 2;  // bit 3-4：己方加密等级（即对方干扰波难度等级），开局为 1，最高为 3
...
struct __attribute__((packed)) SentryCmd{
  ...
  uint32_t remote_exchange_17mm_count : 4;  // bit 13-16：远程兑换发弹量次数，开局为0，单调递增，每次加1
  uint32_t remote_exchange_blood_count : 4;  // bit 17-20：远程兑换血量次数，开局为0，单调递增，每次加1
  uint32_t sentry_change_pose : 2;  // bit 21-22：哨兵修改当前姿态指令，1 为进攻姿态，2 为防御姿态，3 为移动姿态，默认为 3；修改此值即可改变哨兵姿态。
  uint32_t confirm_energy_activated : 1;  // bit 23：哨兵机器人是否确认使能量机关进入正在激活状态，1 为确认。默认为 0。
