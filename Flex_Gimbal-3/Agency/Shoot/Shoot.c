	...
	switch(SHOOT.Action){
		...
		case SHOOT_NORMAL:
				if(Add_Angle_Flag == 1){
					...
					PID_Control_Smis(Pluck_Motor.Measure.angle,SHOOT.Ref_2006_Angle,&Pluck_Place_PIDS,Pluck_Motor.Measure.RoSpeed);
					PID_Control(Pluck_Motor.Measure.RoSpeed, SHOOT.Ref_2006, &Pluck_Speed_PID);  
					limit(Pluck_Speed_PID.pid_out, M2006_LIMIT, -M2006_LIMIT);
					Can1Send_Shoot[0] = (int16_t)Pluck_Speed_PID.pid_out;  					
				}
				else if(Add_Angle_Flag == 0){
