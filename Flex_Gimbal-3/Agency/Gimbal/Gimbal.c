...
void GimbalInit(){
	...
	PID_init(&Gimbal_Place_pid_Yaw[INIT],16384,0,0,3.0f,0,0,0,0);	
	PID_init(&Gimbal_Speed_pid_Yaw[INIT],16384,0,0,3.0f,0,0,0,0);	
	PID_init(&Gimbal_Place_pid_Yaw[GYRO],16384,1,0,50,0,200,0,0);	
	PID_init(&Gimbal_Speed_pid_Yaw[GYRO],16384,1,0,5,0.0,0,0,0);	
	PID_init(&Gimbal_Place_pid_Yaw[AIM],16384,0,0,40.0,0,250.0,0,0);	
	PID_init(&Gimbal_Speed_pid_Yaw[AIM],16384,0,0,8.0f,0,0,0,0);	
...
void Gimbal_Send(){
	if(GimbalCtrl == gAim && DeviceState.PC_State == 1 && ReceiveVisionData.data.dis > 0.1f){
		Can2Send[2] = (int16_t)(Gimbal_Speed_pid_Yaw[AIM].Output + ReceiveVisionData.data.Ref_aYaw);	
		Can2Send[3] = (int16_t)(Gimbal_Speed_pid_Pitch[AIM].Output + ReceiveVisionData.data.Ref_aPitch);	
	} else if(GimbalCtrl == gStop){
    Can2Send[2] = Can2Send[3] = 0;
	} else {
		Can2Send[2] = (int16_t)(Gimbal_Speed_pid_Yaw[GYRO].Output);	
		Can2Send[3] = (int16_t)(Gimbal_Speed_pid_Pitch[GYRO].Output);	
  }
#if GIMBAL_RUN
...
void MedianInit(){
  ...
  limit( Gimbal_Speed_pid_Yaw[INIT].Output,GM6020_LIMIT,-GM6020_LIMIT);
	...
	Can2Send[2] = (int16_t)Gimbal_Speed_pid_Yaw[INIT].Output;
	Can2Send[3] = (int16_t)Gimbal_Place_pid_Pitch[INIT].Output;
	limit(Can2Send[2],GM6020_LIMIT,-GM6020_LIMIT);	
	limit(Can2Send[3],GM6020_LIMIT,-GM6020_LIMIT);	
#if GIMBAL_RUN
	DM_MotorSend(&hcan2,0x3FE,Can2Send);
#endif	
    ...
    switch (RxHeader[1].StdId){
			...
			case 0x102:
						...
						memcpy(&Chassis_data_Rx.bullet_speed,&CAN2_Rxbuff[4],sizeof(float));
										Feed_Dog(&Down_Dog);
				break;
			case 0x303 : DM4310_Receive(&GimYaw,CAN2_Rxbuff);
					Feed_Dog(&Gimbal_Dog[YAW]);
				break;
			case 0x304 : DM4310_Receive(&GimPitch,CAN2_Rxbuff);
