...
static int DM_R_Flag = 0;
void DM4310_Receive(DM4310_TypeDef *Dst, uint8_t *Data){
	Dst->LsatMechAngle = Dst->MchanicalAngle;
	Dst->MchanicalAngle = ((uint16_t)Data[0]) << 8 | Data[1];
	Dst->Speed = 				    (int16_t)(Data[2] << 8 | Data[3]) / 100.0f;
	Dst->TorqueCurrent =    (int16_t)(Data[4] << 8 | Data[5]) / 1000.0f;
	Dst->temp = 	 (int16_t)Data[6];
	Dst->PCBtemp = (int16_t)Data[7];

	int16_t diff = Dst->MchanicalAngle - Dst->LsatMechAngle;
	if(diff != Dst->MchanicalAngle) DM_R_Flag = 1;
	if(DM_R_Flag == 1){
		if (diff > 4000)
				Dst->r--;
		if (diff < -4000)
	...
	Dst->ContinueMechAngle = Dst->r * 8192 + Dst->MchanicalAngle;
	Dst->Angle_DEG = Dst->ContinueMechAngle * 0.0439453125f;
