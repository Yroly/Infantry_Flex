...
void RotationMatrix_update(struct MAHONY_FILTER_t *mahony_filter)
{
    ...
    mahony_filter->rMat[2][2] = 1.0f - 2.0f * (q0q0 + q3q3);
}
...
void mahony_output(struct MAHONY_FILTER_t *mahony_filter)
{
    ...
    mahony_filter->roll = -asinf(mahony_filter->rMat[2][0])* 57.295779513f; 
    mahony_filter->pitch = atan2f(mahony_filter->rMat[2][1],-mahony_filter->rMat[2][2])* 57.295779513f ;
    mahony_filter->yaw = 	atan2f(mahony_filter->rMat[1][0], mahony_filter->rMat[0][0])* 57.295779513f ;
	
	    // get Yaw total, yaw???????360,????????????(????)
    if (mahony_filter->yaw - mahony_filter->YawAngleLast > 180.0f)
    {
        mahony_filter->YawRoundCount--;
    }
    else if (mahony_filter->yaw - mahony_filter->YawAngleLast < -180.0f)
    {
        mahony_filter->YawRoundCount++;
    }
    mahony_filter->YawTotalAngle = 360.0f * mahony_filter->YawRoundCount + mahony_filter->yaw;
}
